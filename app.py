...
def init_images(global_state, img):
    ...
    if img:
        # do e4e inversion instead of loading the latent from disk
        print("no load !!")
        with torch.no_grad():
            img = image_processor.preprocess_image(img, is_batch=False)
            img = img.to(device)
            with torch.no_grad():
                latents, skips = styleres.encoder(img)
                w_pivot = latents
...
def process_image(image, method,  edit, factor, is_align_checked, latent=False):
    ...
    image = styleres.edit_images(image, cfg, latent)
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("DragGAN"):
        ...
        # init image
        global_state = init_images(global_state, None)
        print("start of the draggan tab")
        ...
        with gr.Row():
            ...
            with gr.Row():
                ...
                with gr.Column(scale=3):
                    ...
                    # Show Result Button
                    with gr.Row():
                        show_result_button = gr.Button("Show Result")
                ...
                with gr.Column(scale=8):
                    form_image = ImageMask(
                        ...
                        brush_radius=20).style(
                            width=500, # 768
                            height=500)  # NOTE: hard image size code here.
        ...
        def on_change_pretrained_dropdown(pretrained_value, global_state, draggan_img_input):
            ...
            init_images(global_state, draggan_img_input)
        ...
        form_pretrained_dropdown.change(
            ...
            inputs=[form_pretrained_dropdown, global_state, draggan_img_input],
            outputs=[global_state, form_image],
        )

        def on_click_reset_image(global_state, draggan_img_input):
            """Reset image to the original one and clear all states
            1. Re-init images
            2. Clear all states
            """

            print("in reset:", draggan_img_input)
        ...
        form_reset_image.click(
            ...
            inputs=[global_state, draggan_img_input],
            outputs=[global_state, form_image],
        )

        # Update parameters
        def on_change_update_image_seed(seed, global_state, draggan_img_input):
        ...
        form_seed_number.change(
            ...
            inputs=[form_seed_number, global_state, draggan_img_input],
            outputs=[global_state, form_image],
        )

        def on_click_latent_space(latent_space, global_state, draggan_img_input):
        ...
        form_latent_space.change(on_click_latent_space,
                                inputs=[form_latent_space, global_state, draggan_img_input],
                                outputs=[global_state, form_image])
        
        def on_click_show_result(input_image):
            edited_image = process_image(input_image, "DragGAN", "any", 0, False, True)
            return edited_image

        show_result_button.click(on_click_show_result, inputs=draggan_img_input, outputs=draggan_img_output)
